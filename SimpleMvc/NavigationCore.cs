...
namespace SimpleMvc
{
    public class NavigationCore : INavigator
    {
        ...
        public void Navigate(string a_controllerName, string a_actionName, RouteDictionary a_routeValues)
        {
            ...
            CallAction(controller, a_actionName, a_routeValues);
        }
        ...
        /// <param name="a_routeValues">Values.</param>
        /// <exception cref="ArgumentNullException">Thrown if "<paramref name="a_actionName"/>" is null.</exception>
        public void Navigate<TController>(string a_actionName, RouteDictionary a_routeValues = null)
        {
            #region Argument Validation
        ...
        /// <exception cref="ArgumentNullException">Thrown if <paramref name="a_actionName"/> is null.</exception>
        public void Navigate(string a_controllerName, string a_actionName, dynamic a_routeValues = null)
        {
            #region Argument Validation
            ...
            var routeValues = a_routeValues is null ? null : new RouteDictionary(a_routeValues);

            var controller = _mvc.ResolveController(a_controllerName);

            CallAction(controller, a_actionName, routeValues);
        }
        ...
        /// <param name="a_routeValues">Values.</param>
        /// <exception cref="ArgumentNullException">Thrown if "<paramref name="a_actionName"/>" is null.</exception>
        public void Navigate<TController>(string a_actionName, dynamic a_routeValues = null)
        {
            #region Argument Validation
        ...
        public void Navigate<TController, TViewModel>()
        {
            var viewModel = _mvc.Container.Build<TViewModel>();

            var viewModelType = viewModel.GetType();
            var viewModelAttribute = viewModelType.GetCustomAttribute<ViewModelAttribute>();

            var actionName = string.Empty;

            if (viewModelAttribute is not null)
            {
                actionName = viewModelAttribute.ActionName;
            }
            else if (viewModelType.Name.EndsWith("ViewModel", StringComparison.OrdinalIgnoreCase))
            {
                actionName = viewModelType.Name[..^"ViewModel".Length];
            }
            else
            {
                throw new Exceptions.NavigationException($"Cannot resolve navigation target for view model of type '{viewModelType.FullName}'.");
            }

            var controller = _mvc.ResolveController<TController>();

            CallAction(controller, actionName, viewModel);
        }

        public void Navigate<TViewModel>(string a_controllerName)
        {
            var viewModel = _mvc.Container.Build<TViewModel>();
        ...
        private void CallAction<TController>(TController a_controller, string a_actionName, RouteDictionary a_routeValues)
        {
            ...
            if (actionMethod.ReturnType == typeof(ActionResult))
            {
                ...
                var result = actionExpression.Compile()(a_controller);

                _mvc.HandleResult(a_controller, result);
            }
            else if (actionMethod.ReturnType == typeof(Task<ActionResult>))
            {
                var actionExpression = Expression.Lambda<AsyncActionCall<TController>>(
                                            Expression.Call(
                                                Expression.Constant(a_controller),
                                                actionMethod,
                                                paramExpressions),
                ...
                var callAsync = async () =>
                {
                    var result = await actionExpression.Compile()(a_controller);
            ...
            {
                throw new InvalidOperationException("Unsupported action return type.");
            }
        }
        ...
        /// <param name="a_routeValues">Route values.</param>
        /// <returns>Created expression.</returns>
        private void CallAction<TController, TViewModel>(TController a_controller, string a_actionName, TViewModel a_viewModel)
        {
            // Get all action methods in the controler that match the given action name.
            var controllerType = a_controller.GetType();
            var methods = controllerType.GetMethods(BindingFlags.Public | BindingFlags.Instance)
            ...
            foreach (var method in methods)
            {
                ...
                if (parameters.Length != 1 || parameters[0].ParameterType != typeof(TViewModel))
                    continue;

                values.Add(a_viewModel);
                actionMethod = method;
            }
    ...
    public delegate Task<ActionResult> AsyncActionCall<in TController>(TController a_controller);
